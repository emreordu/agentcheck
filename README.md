...
The thin extension uses the same `@agentcheck/core` workflow as the CLI. Its command palette provides Create Checkpoint, Review Changes, Show Findings, and Clear Checkpoint. The AgentCheck activity view groups findings by severity, file items open workspace files, and the status bar shows the latest risk level. Analysis is command-driven; the extension performs no startup scan or background monitoring.
