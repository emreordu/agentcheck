...
import { assessRisk, BUILT_IN_FINDING_IDS, BUILT_IN_RISK_CONTRIBUTION_IDS, FINDING_IDS, REVIEW_REPORT_SCHEMA_VERSION, toReviewReport, type Finding, type ReviewResult } from "./index.ts";

const finding: Finding = { id: FINDING_IDS.accessControlWeakened, severity: "high", category: "semantic-risk", title: "Presentation title", description: "Presentation description", files: ["src/ü space.ts"], evidence: ["safe"] };
...
test("finding presentation and contribution reason do not drive risk identity", () => {
  const changed = { ...finding, title: "Changed title", description: "Changed description" };
